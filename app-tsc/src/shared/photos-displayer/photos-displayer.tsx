...
export const PhotosDisplayer = (props: PropsType) => {
  ...
  const getUrls = async () => {
    const results: Promise<any[]> = Promise.all(
      props.photos.map(photo =>
        getOneUrlPromise(photo).catch(e => {
          console.log("ERROR GET URL PHOTO", e);
          return e as FirebaseStorageError;
        })
      )
    );
