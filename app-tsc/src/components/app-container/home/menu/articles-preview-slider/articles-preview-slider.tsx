...
const ArticlesPreviewSlider = (props: Props) => {
   ...
   const goToArticle = (id) => {
    props.history.push('/articles/' + id)
        ...
        return (
            <div className={'articles-previews-slider scroll scroll-medium contain-scroll no-gap ' + props.backgroundColorClass}>
                {props.articles.map((article, key) => (
                    <div className='slide flex flex-column' key={key} onClick={() => goToArticle(article.id)}>
                ...
                <div className='slide flex flex-column' onClick={goToArticleList}>
                    <div className="title all-articles"><div className="icon icon-double-chevron"></div></div>
                </div>
            </div>
        );
